...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $username = $conn->real_escape_string($_POST['student_name']);
    $password = $conn->real_escape_string($_POST['student_pass']);

    // Example of selecting user data based on username
    // $sqlSelect = "SELECT * FROM user WHERE u_username = '$username'";
    // $resultSelect = $conn->query($sqlSelect);
    $sqlJoin = "SELECT * FROM student  s where s.student_log = '$username';" ; 
    ...
    if ($resultSelect->num_rows > 0) {
        ...
        if ( $userData['student_pass']== $password  ) {
                ...
                session_start();
                $_SESSION["istifadeci"] = $userData['student_id'];
