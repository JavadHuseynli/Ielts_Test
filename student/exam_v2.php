...
function getValidAudioPath($file_path) {
    ...
    $possible_paths = [
        $file_path, '../' . ltrim($file_path, '/\\'), '../../' . ltrim($file_path, '/\\'),
        '../uploads/' . basename($file_path), '../../uploads/' . basename($file_path),
        '../audio/' . basename($file_path), '../../audio/' . basename($file_path)
    ];
...
// FETCH QUESTIONS - Check if user already has assigned questions
$check_assigned = "SELECT DISTINCT id_questions FROM answers
                   WHERE exam_id = :exam_id AND user_id = :user_id";
$stmt_check = $db->prepare($check_assigned);
$stmt_check->bindParam(":exam_id", $exam_id);
$stmt_check->bindParam(":user_id", $user_id);
$stmt_check->execute();
$assigned_questions = $stmt_check->fetchAll(PDO::FETCH_COLUMN);

if (!empty($assigned_questions)) {
    // User already has assigned questions - fetch only those
    $placeholders = str_repeat('?,', count($assigned_questions) - 1) . '?';
    $query = "SELECT qr.id_question_text, qr.question_text, qr.question_score,
              ...
              JOIN question_files qf ON qr.id_read_quest_file = qf.id_read_quest_file
              JOIN question_types qt ON qr.id_question_type = qt.id_quest_type
              WHERE qr.id_question_text IN ($placeholders)
              ORDER BY FIELD(qr.id_question_text, $placeholders)";

    $stmt = $db->prepare($query);
    $params = array_merge($assigned_questions, $assigned_questions);
    $stmt->execute($params);
    $questions = $stmt->fetchAll(PDO::FETCH_ASSOC);
} else {
    // First time - fetch all questions and assign them to user
    $query = "SELECT qr.id_question_text, qr.question_text, qr.question_score,
                     qt.quest_type_name, qt.question_var,
                     qf.file_title, qf.file_path, qf.file_type, qf.id_read_quest_file
              FROM question_read qr
    ...
    // Assign these questions to the user (save to answers table)
    if (!empty($questions)) {
        $now = date('Y-m-d H:i:s');
        $empty_answer = '';
...
<body class="bg-[#f5f7fa] h-screen flex flex-col">
                ...
                <?php foreach ($grouped_questions as $group_key => $group_data): ?>
                    <?php if ($group_data['file_type'] == 'reading' && isset($file_contents[$group_key])): ?>
                        <div class="material-content" data-group="<?php echo $group_key; ?>" data-type="reading" style="display:none;">
                            <div class="max-w-3xl">
                                <h2 class="text-2xl font-bold text-gray-800 mb-4">Reading Passage</h2>
                        ...
                        <div class="material-content" data-group="<?php echo $group_key; ?>" data-type="listening" style="display:none;">
                            <div class="max-w-3xl">
                                <h2 class="text-2xl font-bold text-gray-800 mb-6">
                                    <i class="fas fa-headphones mr-2 text-green-600"></i>Listening Audio
                                </h2>
                                ...
                                <div class="mb-4 p-4 bg-yellow-50 border-l-4 border-yellow-400 rounded-lg">
                                    <div class="flex items-center">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-yellow-700">Bu audio-nu yalnız <strong>5 dəfə</strong> dinləyə bilərsiniz.</p>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="text-sm text-gray-600 font-medium">Dinləmə sayı</p>
                                ...
                                <div class="bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-8 shadow-lg">
                                    <audio preload="metadata" class="w-full mb-4" id="audio_<?php echo $group_key; ?>" data-group="<?php echo $group_key; ?>">
                                    ...
                                    <div class="mt-4 text-center">
                                        <p class="text-sm font-medium text-gray-600" id="remainingMsg_<?php echo $group_key; ?>">
                                            Qalan dinləmə: <span class="text-green-600 font-bold">5</span>
                                        </p>
                                    </div>
                                </div>
                            </div>
    ...
    // Answer input change handler
    document.addEventListener('change', updateQuestionNav);
    document.addEventListener('input', updateQuestionNav);
    ...
    function updateAudioUI(groupKey) {
        ...
        if (msgEl) {
            if (remaining > 0) {
                msgEl.innerHTML = `Qalan dinləmə: <span class="text-green-600 font-bold">${remaining}</span>`;
            } else {
                msgEl.innerHTML = `<span class="text-red-600 font-bold"><i class="fas fa-ban mr-1"></i>Dinləmə limiti doldu!</span>`;
            }
        }
    ...
    document.addEventListener('click', function(e) {
        if (e.target.closest('.play-btn')) {
            ...
            if (currentCount >= MAX_PLAYS) {
                alert('Bu audio-nu artıq 5 dəfə dinləmisiniz. Daha dinləyə bilməzsiniz.');
                return;
            }
            ...
            audio.play().catch(err => {
                ...
                alert('Audio oynadılmadı. Yenidən cəhd edin.');
            });
        }
    ...
    // Prevent direct audio controls usage
    document.addEventListener('DOMContentLoaded', function() {
        document.querySelectorAll('audio[data-group]').forEach(audio => {
            // Disable native controls play event tracking
            audio.addEventListener('play', function(e) {
                const groupKey = this.dataset.group;
                const currentCount = audioPlayCounts[groupKey] || 0;

                // If play was initiated from native controls (not our buttons)
                // we still need to check the limit
                if (currentCount >= MAX_PLAYS) {
                    this.pause();
                    alert('Bu audio-nu artıq 5 dəfə dinləmisiniz. Play düyməsini istifadə edin.');
                }
            });
        });
    });
